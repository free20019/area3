...
public class YYtimeDao {
	public double findmeiyou(int i){
		...
		String sql="select ceil(sum((xiache-shangche)*24*60*60)) yy from HZGPS_CITIZEN.TB_CITIZEN_"+sj+"@TAXILINK45 t" +
		...
		try {
			...
			rs=ps.executeQuery();
			if (rs.next()) {
				BigDecimal  b4   =   new   BigDecimal(rs.getDouble("yy")/a/60+"");  
