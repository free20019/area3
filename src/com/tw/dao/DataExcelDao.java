...
//			sql = new StringBuffer("select g.*,rownum rn from(select t1.*,v1.* from ");
...
//					sql.append(" where p.comp_name =s.comp_name and f.comp_name=s.comp_name )p1");
					...
					String et = condition.getEndTime().replaceAll("-", "");
					String tab = "jjq.JJQ_TJ_"+st.substring(0, 6)+"_DAY@taxilink89";
					sql.append("select t1.*,f.total, rownum rn  from (select zgs ba_name,fgs comp_name,count(distinct cphm) qt,sum(tjcs) ct,sum(jine) * 100 st"
							+ ",sum(szlc) * 10 distance,sum(kslc)*10 empty1,sum(yssc) * 60 time1,sum(dhsj) * 60 waitTime"
...
//						sql.append(" and '浙'||cphm='" + condition.getCardNo() + "'");	
					sql.append("group by fgs,zgs order by comp_name) t1 left join (select count(vehi_no) total, comp_name"
							+ " from HZGPS_TAXI.VW_VEHICLE@TAXILINK v group by comp_name) f on t1.comp_name = f.comp_name");
					con = DataBese.getConnectionOfOracle();
					try {
						ps = con.prepareStatement(sql.toString());
						rs = ps.executeQuery();
				...
				public List<OperatingData> getGroupsel(Condition condition){
					...
					StringBuffer sql = new StringBuffer("");
					String sj=condition.getStartTime().substring(0,4);
					sql.append("select g.*,rownum rn from(select t1.*,v.* from(select * from  HZGPS_CITIZEN.TB_CITIZEN_"+sj+"@TAXILINK45 t  where shangche>=to_date(");
					sql.append("'"+condition.getStartTime()+"','yyyy-MM-dd hh24:mi:ss') and shangche<=to_date(");
