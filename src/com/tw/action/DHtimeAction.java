...
public class DHtimeAction implements Action{
	...
	private DHtimeDao d=new DHtimeDao();
	...
	public String findshizaiexcle(){
		try {
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均载客等候时间分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"等候时间(时/分)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getWaitting_time()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getWaitting_time()); 
                    	ws.addCell(label);
					}
                	String[] max=halfMonth.getWaittingtimemax();
	            	for (int i = 0; i < max.length; i++) {
						label=new Label(i+1,6,max[i]);
						ws.addCell(label);
					}
	            	String[] min=halfMonth.getWaittingtimemin();
	...
	public DHtimeDao getD() {
		return d;
	}
	public void setD(DHtimeDao d) {
		this.d = d;
	}
