...
import com.tw.dao.YYtimeDao;
import com.tw.entity.HalfMonth;
import com.tw.entity.LOADONLINE;

public class YYtimeAction implements Action{
	...
	private YYtimeDao y=new YYtimeDao();
	...
	public String findshizaiexcle(){
		try {
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均载客时间分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"载客时间(时/分)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getRun_time()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getRun_time()); 
                    	ws.addCell(label);
					}
                	String[] max=halfMonth.getTimemax();
	            	for (int i = 0; i < max.length; i++) {
						label=new Label(i+1,6,max[i]);
						ws.addCell(label);
					}
	            	String[] min=halfMonth.getTimemin();
	...
	public YYtimeDao getY() {
		return y;
	}
	public void setY(YYtimeDao y) {
		this.y = y;
	}
