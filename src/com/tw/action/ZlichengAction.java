...
import com.tw.dao.KSlichengDao;
import com.tw.dao.ShiZaiDao;
import com.tw.dao.ZlichengDao;
import com.tw.entity.HalfMonth;
import com.tw.entity.LOADONLINE;

public class ZlichengAction implements Action{
	...
	private ZlichengDao k=new ZlichengDao();
	...
	public String findzlc(){
		...
		list1=k.findaverage(time);
		halfMonth=h.findyingyunmaxmin(time);
		return SUCCESS;
	}
	public String findzlcexcle(){
		...
		try {
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均行驶总里程分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"总里程(时/公里)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getZlc()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getZlc()); 
                    	ws.addCell(label);
					}
                	String[] max=halfMonth.getNoloadmax();
	            	for (int i = 0; i < max.length; i++) {
						label=new Label(i+1,6,max[i]);
						ws.addCell(label);
					}
	            	String[] min=halfMonth.getNoloadmin();
	...
	public ZlichengDao getK() {
		return k;
	}
	public void setK(ZlichengDao k) {
		this.k = k;
	}
