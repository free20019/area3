...
public class YYCishuAction implements Action{
	...
	public String findyycishuexcle(){
		try {
			List<LOADONLINE>list=new ArrayList<LOADONLINE>();
			List<List<LOADONLINE>>aa=new ArrayList<List<LOADONLINE>>();
			List<String>list2=new ArrayList<String>();
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均营运次数分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"营运收益(元/时)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getRun_times()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getRun_times()); 
                    	ws.addCell(label);
					}
                	String[] max=halfMonth.getTimesmax();
	            	for (int i = 0; i < max.length; i++) {
						label=new Label(i+1,6,max[i]);
						ws.addCell(label);
					}
	            	String[] min=halfMonth.getTimesmin();
	...
	public YYcishuDao getY() {
		return y;
	}
	public void setY(YYcishuDao y) {
		this.y = y;
	}
