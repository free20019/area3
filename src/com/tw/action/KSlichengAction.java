...
public class KSlichengAction implements Action{
	...
	private KSlichengDao k=new KSlichengDao();
	...
	public String findshizaiexcle(){
		...
		try {
			...
			String dateStr2 = new Timestamp(date.getTime()).toString();
				...
				WritableSheet ws = wwb.createSheet("单车平均空驶里程分析",0);//创建Excel工作表 指定名称和位置 
				Label label ;
            	label = new Label(0,0,"空驶里程(公里)"); 
            	ws.addCell(label);
            	...
            	ws.addCell(label);
                	...
                	for (int i = 0; i < shangnian.size(); i++) {
                    	label = new Label(i+1,9,shangnian.get(i).getNo_load_mileage()); 
                    	ws.addCell(label);
					}
                	List<LOADONLINE>shangyue=loadonline.getYy().get(1);
                	for (int i = 0; i < shangyue.size(); i++) {
                		
                    	label = new Label(i+1,8,shangyue.get(i).getNo_load_mileage()); 
                    	ws.addCell(label);
					}
	...
	public KSlichengDao getK() {
		return k;
	}
	public void setK(KSlichengDao k) {
		this.k = k;
	}
