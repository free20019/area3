...
<script type="text/javascript">
	...
	function initialize() {
		var position=new AMap.LngLat(120.16378,30.25840);//创建中心点坐标
		 map=new AMap.Map("data-div",{center:position,level:15,resizeEnable:true});//创建地图实例
	...
	function dingwei(){
		if($("#jd").val()==""||$("#wd").val()==""){
			...
			if(dwmarker!=null){
				...
				map.setCenter(new AMap.LngLat($("#jd").val(),$("#wd").val()));
			}else{
...
function addCluster(tag)
{
	...
	if(tag==1) {
		...
		map.plugin(["AMap.MarkerClusterer"],function(){
			cluster = new AMap.MarkerClusterer(map,markers,{minClusterSize:6,styles:sts,gridSize:60});
		});
	}
	else {
		map.plugin(["AMap.MarkerClusterer"],function(){
			cluster = new AMap.MarkerClusterer(map,markers,{gridSize:120,maxZoom:17,minClusterSize:6});
		});
	}
}
...
//	ngeocoder(mdt.longi,mdt.lati);
	...
	map.setCenter(new AMap.LngLat(mdt.longi,mdt.lati));
