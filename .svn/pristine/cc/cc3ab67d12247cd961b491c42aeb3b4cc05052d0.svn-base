...
public class DHtimeDao {
	public double findmeiyou(int i){
		...
		String sql="select sum((to_number(substr(denghou,-6,2))*3600)+(to_number(substr(denghou,-4,2))*60)+(to_number(substr(denghou,-2,2))))" +
				"  denghou from HZGPS_CITIZEN.TB_CITIZEN_"+sj+"@TAXILINK45 t" +
		...
		try {
			...
			rs=ps.executeQuery();
			if (rs.next()) {
				BigDecimal  b4   =   new   BigDecimal(rs.getDouble("denghou")/a/60+"");  
