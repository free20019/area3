				...
				<div>
					...
					<section style="width:180px;float:left;">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input id="nolinestime" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})()" placeholder="--请选择开始时间--"/>
										</label>
								</section>
								<section style="width:180px;float:left;">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input id="nolineetime" type="text" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})()" placeholder="--请选择结束时间--"/>
										</label>
								</section>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#nolineetime").val(setformatnewlc(mydate));
		...
		$.ajax({
			...
			success:function(json){
					...
					for(var i=0;i<area.length;i++){
						$("#noareaselect").append("<option value='"+area[i].ba_id+"'onclick='findcomp(this.value);'>"+area[i].ba_name+"</option>");
					}
			},
			error:function(){
	...
	function findcomp(obj){
		$.ajax({
			...
			$("#nocompselect").empty();
					...
					for(var i=0;i<area.length;i++){
						$("#nocompselect").append("<option value='"+area[i].comp_id+"' >"+area[i].comp_name+"</option>");
					}
			},
			error:function(){
