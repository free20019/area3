...
//查询月报表
	...
	//疑似模子出租信息统计
	public List<TaxiInfo>findTaxiInfo(String time,String comp_name,String vehi_no){
		List<TaxiInfo>list=new ArrayList<TaxiInfo>();
		String sql="select * from TB_TAXI_MODEL t,HZGPS_TAXI.VW_VEHICLE@TAXILINK v" +
				" where t.vehicle_num=v.vehi_no and operation_times<>0" +
				" and begintime>=to_date('"+time+" 00:00:00','yyyy-MM-dd hh24:mi:ss')" +
				" and begintime<= to_date('"+time+" 23:59:59','yyyy-MM-dd hh24:mi:ss') ";
		if(comp_name!=null&&comp_name.length()>0){
			sql+=" and comp_name like '%"+comp_name+"%'";
		}
		if(vehi_no!=null&&vehi_no.length()>0){
			sql+=" and vehi_no like '%"+vehi_no+"%'";
		}
		...
		try {
			...
			while(rs.next()){
				...
				t.setAddress(rs.getString("address")==null?"":rs.getString("address"));
				t.setBegintime(rs.getString("begintime").substring(0, 19));
				t.setEndtime(rs.getString("endtime").substring(0, 19));
				t.setComp_name(rs.getString("comp_name"));
				t.setOperation_num(rs.getString("operation_times"));
				t.setVehi_no(rs.getString("vehi_no"));
				list.add(t);
			}
			rs.close();
			ps.close();
