...
public class ZlichengDao {
	public double findmeiyou(int i){
		...
		String sql="select sum(kongshi)+sum(jicheng) zlc  from HZGPS_CITIZEN.TB_CITIZEN_"+sj+"@TAXILINK45 t" +
	...
	public List<LOADONLINE>findaverage(String time){
		...
		try {
			...
			while(rs.next()){
				...
				l.setZlc(Double.parseDouble(rs.getString("actual_load_mileage"))+Double.parseDouble(rs.getString("no_load_mileage"))+"");
				list.add(l);
			}
			rs.close();
			ps.close();
