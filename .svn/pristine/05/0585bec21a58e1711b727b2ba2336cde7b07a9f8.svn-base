...
//			System.out.println("vvvvvv="+v.size());
	...
	public static List<Vehicle> getsub(List<Vehicle> vehis){
	      ...
	      // System.out.println(place);
	       ...
	       for(int i=0;i<ps.length;i++){
	    	   vehis.get(i).setLati(Double.parseDouble(ps[i].split(",")[1]));
	    	   vehis.get(i).setLongi(Double.parseDouble(ps[i].split(",")[0]));
			}
	       return vehis;
	}
	...
	public Vehicle getone(Vehicle vehis){
       ...
       String place= zuobiao.getText();
      // System.out.println(place);
    	   vehis.setLati(Double.parseDouble(place.split(",")[1]));
    	   vehis.setLongi(Double.parseDouble(place.split(",")[0]));
       return vehis;
}
}
