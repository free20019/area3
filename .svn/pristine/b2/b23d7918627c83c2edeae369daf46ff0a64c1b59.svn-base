...
public class KSlichengDao {
	public int findmeiyou(int i){
		...
		String sql="select sum(kongshi) kongshi from HZGPS_CITIZEN.TB_CITIZEN_"+sj+"@TAXILINK45 t" +
		...
		ResultSet rs=null;
		Connection con=null;
		SSlichengDao s=new SSlichengDao();
		int a=s.vhicno(i, stime, etime);
		try {
			con=DataBese.getConnectionOfOracle();
			ps = con.prepareStatement(sql);
			rs=ps.executeQuery();
			if (rs.next()) {
				totalpage=rs.getInt("kongshi")/a;
			}
			rs.close();
			ps.close();
			con.close();
